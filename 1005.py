...
def select_feat(train_data, valid_data, test_data, select_all=True):
    y_train, y_valid = train_data[:,-6], valid_data[:,-6]
    raw_x_train, raw_x_valid, raw_x_test = train_data[:,:-6], valid_data[:,:-6], test_data

    if select_all:
        feat_idx = list(range(raw_x_train.shape[1]))
    else:
        feat_idx = [i for i in range(167)]
...
train_data, test_data = pd.read_csv('data1006.csv', encoding="cp1252").values, pd.read_csv('data_test.csv', encoding="cp1252").values
